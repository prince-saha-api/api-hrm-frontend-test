...
const Index = () => {
  ...
  let apiUrl = `/api/payroll/get-payrolldeduction/?page=${currentPage}&page_size=${pageSize}&column_accessor=${
  ...
  const columns = [
    ...
    {
      // for table display
      accessor: "exempted_for_tax",
      title: "Exempted From Tax",
      // visibleMediaQuery: aboveXs,
      render: ({ exempted_for_tax }) => (exempted_for_tax ? "Yes" : "No"),
      // for export
      key: "exempted_for_tax",
    },
    {
      // for table display
  ...
  const getExportDataUrl = () => {
    let url = `/api/payroll/get-payrolldeduction/?column_accessor=${
      sortStatus?.direction === "desc" ? "-" : ""
    }${sortStatus.columnAccessor}`;
